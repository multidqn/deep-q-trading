...
file = open("sp500Week.csv", "w+")
spTimeserie = pandas.read_csv('sp500Day.csv')
...
for record in records:
    ...
    if(datetime.strptime(nextDate, '%m/%d/%Y') >= (currentDate + delta) or datetime.strptime(nextDate, '%m/%d/%Y') < currentDate ):
        ...
        if(datetime.strptime(nextDate, '%m/%d/%Y') < currentDate):
            file.write(str(records[-1]['Date']) + ',' + "00:00" + ',' + str(opeN) + ',' + str(high) + ',' + str(low) + ',' + str(close) + ',' + str(volume) + '\n')
        else:
            file.write(str(nextDate) + ',' + "00:00" + ',' + str(opeN) + ',' + str(high) + ',' + str(low) + ',' + str(close) + ',' + str(volume) + '\n')
        high = record['High']
        opeN = record['Open']
        low = record['Low']
        volume=0
